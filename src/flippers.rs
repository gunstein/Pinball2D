...
impl Plugin for FlippersPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_startup_system(spawn_flippers.after("walls").label("flippers"))
...
fn spawn_flippers(
    ...
    let shape_flipper : lyon::shapes::Rectangle = lyon::shapes::Rectangle {
        extents: Vec2::new(crate::PIXELS_PER_METER * 0.25, crate::PIXELS_PER_METER * 0.05),
        origin: lyon::shapes::RectangleOrigin::Center
    }.into();

    //Spawn left flipper
    let left_flipper_pos = Vec2::new(crate::PIXELS_PER_METER * -0.2, crate::PIXELS_PER_METER * -0.4);
    ...
    commands
        ...
        .insert(Collider::cuboid(shape_flipper.extents.x/2.0, shape_flipper.extents.y/2.0))
        .insert(Transform::from_xyz(left_flipper_pos.x, left_flipper_pos.y, 0.0))
        .insert(LeftFlipper{
            point_of_rotation: Vec3::new(left_flipper_pos.x -(shape_flipper.extents.x/2.0), left_flipper_pos.y +(shape_flipper.extents.y)/2.0, 0.0),
            curr_angle: 0.0
        });

    //Spawn right flipper
    let right_flipper_pos = Vec2::new(crate::PIXELS_PER_METER * 0.1, crate::PIXELS_PER_METER * -0.4);
    ...
    commands
        ...
        .insert(RigidBody::KinematicPositionBased)
        .insert(Collider::cuboid(shape_flipper.extents.x/2.0, shape_flipper.extents.y/2.0))
        .insert(Transform::from_xyz(right_flipper_pos.x, right_flipper_pos.y, 0.0))
        .insert(RightFlipper{
            point_of_rotation: Vec3::new(right_flipper_pos.x +(shape_flipper.extents.x/2.0), right_flipper_pos.y +(shape_flipper.extents.y)/2.0, 0.0),
            curr_angle: 0.0,
        });
}
